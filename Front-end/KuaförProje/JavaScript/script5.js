$(document).ready(function () {
    window.saatleriGuncelle = function () {
        ...
        if (confirm("Çalışma saatlerini güncellemek istediğinizden emin misiniz?")) {
            $.ajax({
                url: 'http://localhost:8080/api/kuafor/2',
                method: 'PUT',
    ...
    function tabloyuGuncelle1() {
        ...
        $.ajax({
            url: 'http://localhost:8080/api/musteri/randevular/1',
            method: 'GET',
            success: function (randevular) {
                ...
                for (const randevu of randevular) {
                    ...
                    var newRow = `
                        <tr>
                            <td>${randevu.kuafor.ad}</td>
                            <td>${randevu.kuafor.soyad}</td>
    ...
    function calismaSaatleriniYukle() {
        $.ajax({
            url: 'http://localhost:8080/api/kuafor/mesai/2',
            method: 'GET',
            success: function(response) {
...
$(document).on('click', '.btn-reddet', function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            // Başarıyla reddedildiğine dair bir bildirim göster
            alert('Randevu başarıyla reddedildi.');
